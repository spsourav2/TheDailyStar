...
this.taxService.getAllSlabType().subscribe((response:ApiResponse)=>{
  ...
  slabIncomeByTaxYearId(id:number){
    if(id==null)
    return;
    this.taxService.getAllSlabIncomeByTaxYearID(id).subscribe((response:ApiResponse)=>{
      if(response.status){
  ...
  slabIncomeBySlabTypeId(id:number){
    if(id==null)
    return;
    this.taxService.getAllSlabIncomeBySlabId(id).subscribe((response:ApiResponse)=>{
      if(response.status){
        this.slabIncomeTax =response.result as SlabIncomeTax[];
      }
    })
