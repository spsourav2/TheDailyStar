...
namespace HRMS.DbContext
{
    public class Leave
    {
        ...
        public static List<LeaveTypeModel> GetLeaveType(int gradeValue, int gender)
        {
            using (var con = new SqlConnection(Connection.ConnectionString()))
            {
                var types = con.Query<LeaveTypeModel>("sp_getLeaveTypeGradeValue", param: new { Grade = gradeValue, gender }, commandType: CommandType.StoredProcedure).ToList();
                return types;
            }
        }
        ...
        public static bool DeleteById(int id)
        {
            var con = new SqlConnection(Connection.ConnectionString());
            {
                var query = $"Delete FROM Leaveinfo Where ID ="+id;
                var result = con.Execute(query) > 0;
                return result;
            }
        }
