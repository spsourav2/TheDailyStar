...
namespace WebApiCore.DbContext.Attendance
{
    public partial class AttendenceApplication
    {
        ...
        public static bool SaveOrUpdateManualAttendence(ManualAttendenceModel attnModel)
        {
            ...
            var conn = new SqlConnection(Connection.ConnectionString());
            string quire = $"INSERT INTO AttendencDetails(PerID,EmpCod,AttnDate,AttnTime,TYPEE,Hourr,Minutee,Secondd,DDMMYYYY,MachineName,CompanyID,punch_time) VALUES" +
                $"({attnModel.PerID},{attnModel.EmpCod},'{attnModel.AttnDate}','{attnModel.AttnTime = AttnTime}'," +
                $"{attnModel.TYPEE},{attnModel.Hourr = decimal.Parse(hour)},{attnModel.Minutee = decimal.Parse(minite)},{attnModel.Secondd = decimal.Parse(second)},'{attnModel.DDMMYYYY}',{attnModel.MachineName = "'Success'"},{attnModel.CompanyID},'{attnModel.punch_time}')";
        ...
        public static bool UpdateManualAttendence(ManualAttendenceModel attnModel)
        {
            ...
            string quire = $"UPDATE AttendencDetails set PerID={attnModel.PerID}, EmpCod='{attnModel.EmpCod}',AttnDate='{attnModel.AttnDate}'," +
                ...
                $"Minutee={attnModel.Minutee = decimal.Parse(minite)},Secondd={attnModel.Secondd = decimal.Parse(second)},DDMMYYYY='{attnModel.DDMMYYYY = attnModel.DDMMYYYY}'," +
                $"MachineName={attnModel.MachineName = "'Success'"},CompanyID={attnModel.CompanyID}, punch_time='{attnModel.punch_time}'  WHERE ID={attnModel.ID}";
            int rowAffected = conn.Execute(quire);
            return rowAffected > 0;
        }
