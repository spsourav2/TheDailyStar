...
namespace WebApiCore.DbContext.HR
{
    public static class Employee
    {
        ...
        {
            var con = new SqlConnection(Connection.ConnectionString());
            var query = $"SELECT EmpCode,CONVERT(int, EmpCode) AS Code  FROM EmpGeneralInfo Egi WHERE  Active=1 UNION SELECT EmpCode,CONVERT(int, EmpCode) AS Code FROM EmpGeneralInfo WHERE  Active=1 \r\nORDER BY Code";
